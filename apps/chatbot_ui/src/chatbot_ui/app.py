...
def hitl_popup(task_data: dict):
    ...
    with col1:
        if st.button("✅ Approve", type="primary", use_container_width=True):
            ...
            st.session_state.human_decision = {"approved": True, "feedback": feedback}
            st.rerun()
    with col2:
        if st.button("❌ Reject", use_container_width=True):
            st.session_state.pending_hitl = None
            st.session_state.human_decision = {"approved": False, "feedback": feedback}
            st.rerun()
...
if st.session_state.human_decision is not None:
    ...
    with st.chat_message("assistant"):
        ...
        for line in api_call_stream(
            "post",
            f"{config.API_URL}/send_human_response",
            json={
                "thread_id": session_id,
...
with st.sidebar:
    ...
    with suggestions_tab:
        if st.session_state.used_context:
            for i, item in enumerate(st.session_state.used_context):
...
#     with st.chat_message("assistant"):
#         status, output = api_call("post", f"{config.API_URL}/rag", json={"query": prompt, "thread_id": session_id, "rerank": rerank})
            ...
            if line_text.startswith("data: "):
                ...
                try:
                    ...
                    if output["type"] == "final_answer":
                        ...
                        message_placeholder.markdown(answer)
                        break
                    elif output["type"] == "human_interrupt":
